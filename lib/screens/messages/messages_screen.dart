...
import 'package:xlo_mobx/screens/messages/components/ad_tile_message.dart';
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text(chatRoomStore.ad.title ?? 'erro no titulo'),
          //Text(ad.title),
          actions: [
            PopupMenuButton(
        ...
        body: Column(
          ...
          children: [
            AdTileMessage(ad: chatRoomStore.ad),
            Expanded(
                ...
                child: Observer(
                  builder: (_) {
                    return ListView.builder(
                      ...
                      itemBuilder: (_, index) {
                        return MessageTile(
                          message: messageStore.messageList[index],
                        );
                      },
                    );
                  },
            ...
            if (chatRoomStore.ad.status!.index >= AdStatus.SOLD.index)
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    SizedBox(height: 8),
                    Center(child: Icon(Icons.dangerous)),
                    SizedBox(height: 16),
                    Center(
                      child: Text(
  ...
  void deleteChatRoom(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (_) => DialogPlatform(
        title: 'Excluir',
        content: 'Confirma exclusão do Chat \'${chatRoomStore.ad.title}\'?',
        textNoButton: 'Não',
        textYesButton: 'Sim',
  ...
  void showProfile(BuildContext context) {
    ...
    if (user == null) {
      showDialog(
        ...
        builder: (_) => DialogPlatform(
          title: 'Erro',
          content: 'Perfil do usuário ainda não foi carregado.',
          textYesButton: 'OK',
          actionYes: () => Navigator.of(context).pop(),
          textNoButton: '',
          actionNo: null,
        ),
      );
