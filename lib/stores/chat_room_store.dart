...
abstract class _ChatRoomStore with Store {
  ...
  ChatRoom? get chatRoom {
    ...
    if (_ad != null) {
      if (chatRoomList.any((c) => c.adId?.id == _ad!.id)) {
        isNew = false;
        return chatRoomList.firstWhere((c) => c.adId?.id == _ad!.id);
      }
    }
    isNew = true;
    return null;
  ...
  @computed
  User? get destination => users.firstWhere(
    (user) => user != null && user.id != sender.id,
    orElse: () => null,
  );
  ...
  Future<void> _getChatRoomList() async {
    try {
      final chatRooms = await chatRoomRepository.getChatRoomList(
  ...
  Future<ChatRoom?> createChatRoom({required Ad ad}) async {
    try {
      ...
      final ChatRoom chatRoom = await chatRoomRepository.createChatRoom(
        ad: ad,
        users: users,
      );
