...
abstract class _MessageStore with Store {
  ...
  getMessagesList() async {
    try {
      setLoading(true);
      final messages = await messageRepository.getMessagesList(
        chatRoomStore.chatRoom!,
      );
      messageList.clear();
  ...
  @computed
  List<User?> get users {
    if (chatRoomStore != null) {
      return chatRoomStore.users;
    }
  }
  ...
  sendMessage(String message) async {
    if (chatRoomStore.chatRoom == null) {
      ...
      if (!chatRoomStore.chatRoomList.any(
        (e) => e.id == chatRoomStore.chatRoom!.id,
      )) {
    ...
    late final destination = chatRoomStore.destination;
    if (destination == null) {
